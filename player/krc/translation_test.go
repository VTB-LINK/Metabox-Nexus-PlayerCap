...
// 本文件只测酷狗内嵌翻译轨：parseTranslation 解 [language:] 轨，以及 ParsePlainKRC
// 把译文按 [start,dur] 行号对齐进 Line.SubText。
//
// 与网易云的翻译**机制不同**：网易云是独立 tlyric LRC、按毫秒时间戳匹配；酷狗是同一份 KRC
// 内嵌、按行号位置对齐。最容易做错的是对齐——遇到被跳过的 0 词行若不占索引就整轨错位。
//
// 注：汽水音乐的译文是独立字段、不走这条内嵌轨，故对汽水明文 parseTranslation 恒返回 nil。
...
// TestParseTranslationTakesType1 —— 只取中文翻译轨(type=1)，绝不误取罗马音(type=0)。
// 变异自证：把 parseTranslation 里的 `blk.Type != 1` 改成 `!= 0`，本例会拿到 "se n" 而红。
func TestParseTranslationTakesType1(t *testing.T) {
...
// 无 [language:] 标签 → nil；ParsePlainKRC 照常出词、SubText 全空。
func TestParseTranslationAbsent(t *testing.T) {
	if tr := parseTranslation("[ti:x]\n[0,1000]<0,500,0>甲\n"); tr != nil {
		t.Fatalf("无 [language:] 应返回 nil，got %#v", tr)
	}
}

// ParsePlainKRC 端到端：译文按 [start,dur] 行号对齐进 SubText。
// 5 个主行：idx0/1/2 头部(译空)，idx3/4 正文(有译)。
func TestParsePlainKRCSubTextAligned(t *testing.T) {
	krc := langTagA + "\n" +
...
// ★ 承重用例：主歌词里夹一个 0 词行（会被 ParsePlainKRC 跳过、不产出 Line），但它在译轨里**照样占一格**。
// rawIdx 必须为它自增，否则其后的译文整轨错位一格。
// 变异自证：把 ParsePlainKRC 里的 `rawIdx++` 移到 append 之前的 `if len(words)==0 { continue }` 之后，
...
func TestParsePlainKRCSubTextSkipsBlankLineWithoutMisalign(t *testing.T) {
	krc := langTagB + "\n" +
		"[0,1000]<0,500,0>千<500,500,0>本\n" + // idx0
		"[1000,1000]<0,500,0>词<500,500,0>：\n" + // idx1
		"[2000,1000]<0,500,0>曲<500,500,0>：\n" + // idx2
		"[3000,1000]\n" + // idx3：0 词空行 → 被 ParsePlainKRC 跳过，但占译轨一格
		"[32119,2870]<0,900,0>大<900,900,0>胆\n" + // idx4
		"[34989,3180]<0,900,0>磊<900,900,0>落\n" // idx5
	lines := ParsePlainKRC(krc)
	if len(lines) != 5 {
