...
│   │   ├── cloudmusic.go  # 主轮询循环、Redux/DOM 对齐、本地时钟同步、seek 检测
│   │   ├── cdp/
...
│       ├── qqmusic.go     # 主轮询循环、快速计时器插值、暂停/恢复/seek 检测
...
8. 播放器切换时，`switchTo()` 同步调用 `NotifySubscribersFullState()`，向根订阅者推送 `player_switch` 事件 + 新播放器**已缓存的**初始状态（缓存为空则仅推 `player_switch`）。函数返回实际推送过的事件类型集合，用于后续 `switchSkip` 抑制（避免实时事件与 FullState 重复，但不吞首次数据）。当根 WS 新连入且当前 `activePlayer` 为空时，`buildInitEvents()` 会立即返回一个 `player_clear` 初始事件，避免静默连入。
...
| seek 检测 | `检测到回跳: %.2fs → %.2fs` / `检测到前跳: %.2fs → %.2fs` | — |
| 暂停/恢复 | `暂停 @ %.2fs` / `恢复 @ %.2fs` | `暂停 @ 45.20s` |

**规则：**
- 禁止混用中英文表述同一语义（如同时出现 "Fetched lyrics" 和 "歌词加载完成"）。
- 来源标注放在括号内作为后缀（如 `(CDP)`、`(Redux)`、`(API)`），无特殊来源时省略。
- 新播放器接入时，参照上表对齐已有播放器的输出格式。
- seek 与恢复在同一 tick 同时发生时，只发送一次 `playback_resume`，避免下游重复处理。
    ...
    runSession():              // 会话：Ticker 驱动
      ticker.C:
        cdp.Extract()          // 执行 JS → 取 Redux + DOM 数据
                detectSongChange()     // 歌名变化 → 强制 Redux 刷新 → 仅采纳与当前 DOM 对齐的 Redux ID/歌词/封面
    ...
    runSession():              // 会话：Hook 注入 → 轮询
      ...
      pollLoop():
        ...
        detectSongChange()     // 歌名变化 → fetchLRC + fetchCoverURL
                hybridInterpolation()  // 快速计时器锚点 + 本地时钟插值
        detectPause/Resume     // 快速计时器停滞 1s → 暂停
                detectSeek()           // 快速计时器异常跳变（前跳/回跳）> 2s → seek
...
| QQMusic | `progress 回退 > 2000` 或 `progress 前进量 > 实际经过时间 + 2000`（快速计时器跳变） | 2 秒 |
